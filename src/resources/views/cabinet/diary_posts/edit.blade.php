    ...
    <x-slot name="content">
        <x-card-general>
            ...
            <x-links-head>
                ...
                <a href="{{ route('cabinet.diary_posts.create') }}" class="btn btn-link">{{ __('Add New Post') }}</a>
                <a href="{{ route('cabinet.diary_posts.show', $post->id) }}" class="btn btn-link">{{ __('View Post') }}</a>
                <x-modal :id="'post-'.$post->id" :title="__('Delete post')" :submit_text="__('Delete')">
                    <form method="post" action="{{ route('cabinet.diary_posts.destroy', [$post->id]) }}" id="post-{{ $post->id }}-form">
                        @csrf
                        @method('delete')
                        <p class="text-start">
            ...
            <div>
                <form method="post" action="{{ route('cabinet.diary_posts.update', $post->id) }}">
                    @csrf
                    @method('put')
