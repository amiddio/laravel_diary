            ...
            <table class="table">
                ...
                @foreach($posts as $post)
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-link" href="{{ route('cabinet.blog_posts.edit', [$post->id]) }}">{{ __('Edit') }}</a>

                            <x-modal :id="'post-'.$post->id" :title="__('Delete post')" :submit_text="__('Delete')">
                                <form method="post" action="{{ route('cabinet.blog_posts.destroy', $post->id) }}" id="post-{{ $post->id }}-form">
                                    @csrf
                                    @method('delete')
                                    <p>
