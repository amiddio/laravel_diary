<x-app-layout>
    ...
    <x-slot name="content">
        <x-card-general>
            ...
            <x-links-head>
                <a href="{{ route('categories.create') }}" class="btn btn-link">{{ __('Add Category') }}</a>
            </x-links-head>
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    <td>
                        <a class="btn btn-link" href="{{ route('categories.edit', [$category->id]) }}">{{ __('Edit') }}</a>

                        <x-modal :id="'cat-'.$category->id" :title="__('Delete category')" :submit_text="__('Delete')">
                            <form method="post" action="{{ route('categories.destroy', [$category->id]) }}" id="cat-{{ $category->id }}-form">
                                @csrf
                                @method('delete')
                                <p>
